...
import { GUNS, playerFireCooldowns } from "../data/guns";
export function shoot(player, currentGun) {
    ...
    if (arrow) {
        ...
        if (projectileComponent) {
            ...
            projectileComponent.shoot(Vector3Utils.scale(direction, 2)); // Scale velocity as needed
        }
    }
    // Set cooldown
    playerFireCooldowns.set(player.id, gun.fireRate);
}
export function reload(player, currentGun) {
    const gun = GUNS.find(g => g.id === currentGun.id);
    if (!gun)
        return false;
